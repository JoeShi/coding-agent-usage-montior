...
async fn main() {
    ...
    match providers::ark::fetch_usage_details(&http, &yesterday, &yesterday, "Day").await {
        Ok(rows) => println!("{} rows; first 3:\n{}", rows.len(),
            serde_json::to_string_pretty(&rows.iter().take(3).collect::<Vec<_>>()).unwrap()),
        Err(e) => println!("error: {:?}", e),
    }

    println!("\n=== Ark via env AK/SK (validation-only signing path) ===");
    match (std::env::var("VOLCENGINE_ACCESS_KEY"), std::env::var("VOLCENGINE_SECRET_KEY")) {
        (Ok(ak), Ok(sk)) => {
            let creds = Credentials { access_key: ak, secret_key: sk, session_token: None };
            let body = "{}";
            let query = vec![
                ("Action".to_string(), "GetAFPUsage".to_string()),
                ("Version".to_string(), "2024-01-01".to_string()),
            ];
            let signed = volc_sigv4::sign(&creds, "cn-beijing", "ark", &SignRequest {
                method: "POST",
            ...
            match resp {
                Ok(r) => {
                    ...
                    match v.pointer("/ResponseMetadata/Error") {
                        ...
                        None => println!("accepted: PlanType={:?}",
    ...
    match entry {
        Ok(e) => {
            ...
            match r {
                Ok(v) => println!("ok: wrote, read back ({}), deleted", if v == "probe-secret" { "match" } else { "MISMATCH" }),
                Err(e) => println!("error: {e}"),
            }
        }
        Err(e) => println!("error: {e}"),
