...
续期子进程 MUST 在 GUI 启动的极简 PATH 环境下可运行：arkcli 的可执行入口是 `#!/usr/bin/env node` 脚本，spawn 时系统 SHALL 将 arkcli 二进制所在目录注入子进程 PATH（node 与 arkcli 同目录，覆盖 nvm/homebrew/usr-local 等安装方式），否则子进程因找不到 node 退出 127，续期静默失效。
...
#### Scenario: GUI 环境下续期可用

- **WHEN** 应用从 Finder/launchd 启动（PATH 极简、不含 node），sts.json 已过期且 identity 有效
- **THEN** 续期子进程正常执行（node 可从注入的 PATH 解析），续期成功，不提示重新登录
